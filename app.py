...
st.markdown("Envie **uma fatura (PDF)** e **dois relatórios de geração (XLS/XLSX)** para análise exata do período da leitura.")
...
def extrair_dados_pdf(texto):
    ...
    credito_match = re.search(r"Saldo Acumulado.*?Todos os Períodos\s+(\d{1,5})", texto)
...
if fatura and geracoes and len(geracoes) == 2:
    ...
    data_inicio, data_fim = extrair_datas_leitura(texto)
    energia_consumida, energia_injetada, creditos = extrair_dados_pdf(texto)

    if not data_inicio or not data_fim:
        st.error("❌ Não foi possível identificar as datas de leitura automaticamente.")
    else:
        ...
        if desempenho < 70:
            st.markdown("- 🌤️ Baixo desempenho de geração: possível problema de dimensionamento.")
        if eficiencia < 70:
    ...
    st.info("📌 Envie uma fatura e exatamente **2 arquivos de geração** para iniciar a análise.")
