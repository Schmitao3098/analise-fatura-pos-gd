...
data_inicio = st.date_input("\U0001F50D Data da leitura anterior (início do período):")
data_fim = st.date_input("\U0001F4C5 Data da leitura atual (fim do período):")
...
if fatura and geracoes and len(geracoes) == 2:
    ...
    st.write(f"🗓️ Período informado: **{data_inicio.strftime('%d/%m/%Y')} a {data_fim.strftime('%d/%m/%Y')}**")
    ...
    st.info("📌 Envie uma fatura, exatamente **2 arquivos de geração**, e informe as **datas** para análise.")
